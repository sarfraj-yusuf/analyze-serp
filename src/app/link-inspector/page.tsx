...
import { Link2, Sparkles, ArrowLeft, Search, Globe } from 'lucide-react';
    ...
    <div className="min-h-screen flex flex-col bg-[var(--bg-main)] text-slate-900 dark:text-gray-100 selection:bg-emerald-500 selection:text-black transition-colors duration-200">
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        ...
        <div className="flex items-center gap-2 text-xs">
          ...
          <span className="text-slate-900 dark:text-white font-bold">Deep Link & Affiliate Inspector</span>
        </div>
        ...
        <div className="text-center space-y-3 max-w-3xl mx-auto py-4">
          <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold bg-amber-500/10 text-amber-700 dark:text-amber-300 border border-amber-500/20">
            ...
            <span>Monetization & Citation Intelligence</span>
          </div>

          <h1 className="text-3xl sm:text-4xl font-black text-slate-900 dark:text-white tracking-tight">
            Deep Link & <span className="gradient-text">Affiliate Footprint Inspector</span>
          </h1>

          <p className="text-sm text-slate-600 dark:text-gray-400 leading-relaxed">
            Classify anchor text categories (Keyword-Rich, Branded, Generic) and automatically detect competitor affiliate monetization tracking links (Amazon, ShareASale, Impact, CJ Affiliate).
          </p>
        </div>
        ...
        <div className="glass-panel p-6 rounded-2xl border border-slate-200 dark:border-white/10 max-w-3xl mx-auto space-y-4">
          <form onSubmit={handleInspectUrl} className="flex flex-col sm:flex-row items-center gap-3">
            <div className="relative flex-1 w-full">
              ...
              <input
                type="text"
                placeholder="Enter URL to inspect link footprint (e.g. https://ahrefs.com/blog/on-page-seo/)"
                value={urlInput}
